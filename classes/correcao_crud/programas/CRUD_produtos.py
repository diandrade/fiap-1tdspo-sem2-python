...
def main():
    ...
    while (resp == 1):
        ...
        print("13-Ordenar dados pelo valor do produto")
        opcao = int(input("Digite a opcao desejada (1 a 13): "))
        if (opcao >= 1 and opcao <= 13):
            match opcao:
                ...
                case 13:
                    ordenar_produtos_valor(lista_produtos)
        else:
...
def ordenar_produtos_valor(lista_produtos):
    tam = len(lista_produtos)
    for i in range(tam-1, 0, -1):
        for j in range(0, i):
            if (lista_produtos[j]['Valor'] > lista_produtos[j + 1]['Valor']):
                aux = lista_produtos[j]
                lista_produtos[j] = lista_produtos[j + 1]
                lista_produtos[j + 1] = aux
